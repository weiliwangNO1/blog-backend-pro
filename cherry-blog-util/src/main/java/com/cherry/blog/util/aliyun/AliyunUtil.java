package com.cherry.blog.util.aliyun;
...
import com.cherry.blog.util.properties.AliyunProperties;
...
public final class AliyunUtil {
     ...
     * @return
     */
    public static Result uploadFileToOss(PlatformEnum platformEnum, MultipartFile file,
                                         AliyunProperties aliyun ) {
        // 上传
        // 上传文件所在目录名，当天上传的文件放到当天日期的目录下。
        String folderName = platformEnum.name().toLowerCase() + "/" + DateFormatUtils.format(new
                Date(), "yyyyMMdd");
        ...
        try {
            ...
            // 上传文件到OSS 并响应结果
            PutObjectResult putObjectResult = ossClient.putObject(aliyun.getBucketName(),
